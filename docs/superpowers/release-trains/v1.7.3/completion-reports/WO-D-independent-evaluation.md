...
状态：WO-D evaluation harness 的剩余 P1 已完成并本地提交；没有 supplied Feature candidate、没有 B/C integrated candidate、没有 browser acceptance。因此没有任何 candidate 被接受或标记为 release-ready。
...
- strict suite 不执行整个目录后容忍 meta skip，而是只显式运行六个 candidate-facing 文件。它以 temporary raw JUnit XML 得到结构化 `strict-suite.junit.summary.json`；原始 XML 会在 `finally` 删除。JUnit 缺失、不可解析、任意 skip、非 candidate testcase 或任一 required result 零 testcase 都失败。
...
- raw candidate-facing pytest stdout/stderr 不再写入 artifact 或 manifest。`strict-suite.output.json` 与 `strict-runner.output.json` 只保留全量 stream SHA-256、上限 65,536 bytes 的计数和 truncation flag；候选控制的异常文本也不进入 strict result。模拟 secret 的 guardrail 测试会扫描 artifact tree，确认 sentinel 不出现。
...
本次 remaining-P1 code commit `98be242895cfd2915da029abd24a64af2427de29` 只修改：
...
| 命令 | 实际结果 |
| --- | --- |
| `/Users/jiayuanren/项目规划/.venv/bin/python -m py_compile scripts/collect_v173_lmm_evidence.py tests/evaluation/linear_mixed_effects/strict_runner.py` | exit 0。 |
| `/Users/jiayuanren/项目规划/.venv/bin/python -m pytest tests/evaluation/linear_mixed_effects/test_collector_guardrails.py tests/evaluation/linear_mixed_effects/test_strict_candidate_entrypoint.py -q` | exit 0；`35 passed in 9.71s`。覆盖 stream-sentinel 不持久化、integration branch/SHA/clean guard 和 reservation/no-clobber guard。 |
| `/Users/jiayuanren/项目规划/.venv/bin/python -m pytest tests/evaluation/linear_mixed_effects -q` | exit 0；`54 passed, 27 skipped in 10.84s`。本地 authoring mode 对未提供 candidate-only module 保留 skip；strict mode 不允许它们 skip。 |
| `git diff --check` | exit 0（docs commit 前复核）。 |

此前全量 `pytest tests -q` 的已知 sandbox blocker 未在本次重跑：旧记录为受管环境拒绝 `sandbox-exec` profile（`sandbox_apply: Operation not permitted`）。WO-D 没有修改 sandbox/central code，也没有用 workaround 掩盖该环境问题。
...
- stream hardening 只保证 captured candidate stdout/stderr 与候选控制的 strict error text 不被持久化；它不是全盘 secret scanner，也不保证 candidate 生成的 result/performance JSON 本身不含敏感内容。evidence 必须保存在受控本地目录。
- **P2 残余：** `O_EXCL` reservation 与 no-clobber hard link 可覆盖正常同文件系统并发，但无法用 portable Python API 对恶意同权限进程替换父目录/路径、符号链接攻击、跨主机文件系统异常或崩溃后 stale reservation 给出绝对保证。stale reservation 不会自动清理，必须先人工检查后显式移除；受控目录权限和审计存储仍是必要外部控制。
