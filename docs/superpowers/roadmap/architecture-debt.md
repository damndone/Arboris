...
## D7. 敌对扩展/第三方模型包的冻结执行边界（未来需求，当前不实现）

- **触发条件**：只有在 Workbench 开始接收不受信任的插件、第三方模型包、外部生成
  代码，或进入多人部署/App 分发时，才重新立项。
- **v1.7.3 决定**：本机版本只运行仓库内已审查代码，并通过每次启动显式开启的
  `local_contained` 配置提供 LMM 与 `code.execute`。未接入运行时的 C2 原型及专属
  测试已删除，避免把未来认证系统当成当前产品能力。
- **未来最低要求**：重新写威胁模型；从精确、干净的源码快照执行；使用目标平台真实
  OS 读写/网络/派生进程隔离；启动 canary；绑定可信运行时身份；由父进程生成不可由
  候选伪造的审计收据；任何隔离失败均拒绝执行且不得回退到裸 Python。
- **重新立项规则**：不得直接恢复 v1.7.3 原型。必须先证明产品运行时确实存在这一
  信任边界，再从新的 Context Pack、目标宿主和验收测试开始。
...
7. **D7 敌对扩展冻结执行**（事件触发，不排入普通本机版本；只有新增信任边界时启动）。
