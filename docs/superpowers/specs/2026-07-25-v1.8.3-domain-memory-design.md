...
**状态**：设计已批准；MEM1–MEM3 代码切片、Notebook 接入与 focused tests 已完成；Core Trace 注册、部署策略与版本收口 gate 仍属 Integration/acceptance 工作

**实现边界快照（2026-07-27）**：Project/RunFamily 可重建索引、默认关闭的跨项目使用/迭代开关、作用域/来源授权/批准/有效性/有界检索、候选-only curator、冲突与 review 生命周期，以及 Notebook 展示已落地。候选不会直接成为 active memory；memory 不能改变 capability ranking、evidence、admission 或 execution authorization。
