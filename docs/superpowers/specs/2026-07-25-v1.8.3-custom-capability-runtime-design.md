...
本文件只定义 v1.8.3 的 B0 通用运行时地基。能力需求解析、可信实现优先级、依赖构建、Adapter Factory、算法验证与准入、Notebook 接入和双层领域记忆由两份平级规格定义：
...
[`双层领域记忆`](./2026-07-25-v1.8.3-domain-memory-design.md)
和唯一的
[`Execution Control Plan`](../plans/2026-07-25-v1.8.3-execution-control.md)
为语义与执行边界：
