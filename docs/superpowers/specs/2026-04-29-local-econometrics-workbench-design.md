...
第一版不做云端协作、不做复杂项目级 ETL 建模、不覆盖所有计量方法。重点是跑通“原始数据到可信报告”的闭环，并让每一步可检查、可回滚、可复现。
...
→ 生成图表、表格、诊断结果
→ 混合式报告撰写
→ 导出 HTML / PDF / Word / Notebook / LaTeX
→ 保存所有中间产物、日志、时间戳和备份
```
...
- Econometrics Engine：执行第一版计量方法。
- Visualization Engine：生成描述统计图、相关性图、残差诊断图、时间趋势图和面板概览图。
- Report Engine：组织结构化结果，规则模板生成核心结论，LLM 辅助摘要、解释和润色，并绑定数据证据。
- Artifact Store：保存原始数据副本、清洗版本、合并结果、模型结果、图表、报告、日志和 manifest。
...
HTML 是主报告格式，因为它最适合展示可展开的清洗日志、模型诊断、图表和数据版本。PDF、Word、Notebook 和 LaTeX 从同一套结构化报告内容导出。
...
```json
{
  ...
  "source": "model_results.regression_1.coefficients.x1",
  "confidence": 0.82
}
```
...
- BLOCKER：必须停止或请求用户确认。
- WARNING：可继续，但报告和 manifest 中显著标记。
- INFO：记录即可。
...
- 每个报告结论都能回溯到 manifest、metrics、model result 或 figure。
- 每个自动清洗动作都有记录。
