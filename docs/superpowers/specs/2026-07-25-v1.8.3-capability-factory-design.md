...
**状态**：设计已批准；B0、CORE、B1、CF1–CF4 已完成代码切片与 focused tests；版本收口仍待真实目标主机 canary、受信部署 authority bootstrap、full gate 与 browser acceptance

**实现边界快照（2026-07-27）**：Capability Factory 已具备需求解析、依赖 quarantine/build、Adapter 生成与验证、证据/准入生命周期、Proposal/Risk 绑定、Notebook `model.custom` materialize/confirm-and-execute 控制面，以及明确标记为 `experimental` 的 Darwin local gateway seam。该 seam 默认未安装；只有服务端显式注入 `AuthorizedCapabilityExecutionGateway`、当前 host canary 为 `supported`、报告经过受信 verifier，并且用户确认高风险 `experimental_confirm_and_execute` 后才可进入执行。没有这些条件时保持 fail closed；本快照不把单元测试或当前受限 Codex host 的 `unsupported` 当成真实 Darwin acceptance。

**运行时地基**：
[`v1.8.3 Custom Capability Runtime`](./2026-07-25-v1.8.3-custom-capability-runtime-design.md)

**配套规格**：
[`v1.8.3 双层领域记忆`](./2026-07-25-v1.8.3-domain-memory-design.md)
...
- invocation intent 签发、attestation 验证和结果 envelope；
- Artifact Contract、lineage、Trace、rerun 与 consumer admission；
- 用户确认和撤销。

这些字段不能由作者代码返回，也不能从自由文本推断后直接持久化。

跨 Capability 与 Memory 的本地身份只能由同一个可信身份服务产生：
`LocalProfileIdentity` 和 `ProjectIdentityRevision` 均由服务端签发、不可变且可修订；
目录路径、客户端字段、显示名称或相同内容 hash 都不能充当身份。
Capability Factory 与领域记忆只能消费这些身份，不能各自创建第二套 profile/project identity。

### 2.2 B0 authority/verifier core

B0 只理解经过规范化和认证的通用 claims：

```text
authority trust snapshot
+ invocation / containment / execution / evidence attestation
+ expected binding and current validity cursors
→ verified claims | typed rejection
```

它不启动代码，不解释 capability、输入或输出语义，也不产生 evidence tier、source eligibility 或 supported-host 结论。B0 只验证 producer authority、真实性、版本、完整绑定、有效期、撤销和重放状态；具体 contract 以 digest 绑定。

### 2.3 B1 native containment adapter/broker

B1 在独立可信 helper/broker 中执行不可信代码，负责平台沙箱、密封输入输出、无网络、进程树资源预算、清理和真实宿主 canary。只有 B1 可以基于目标宿主证据签发 containment/execution attestation；后端不可用或保证不足时必须 fail closed。

B1 与 B0 分开验收。Python 包内的私有函数不能替代独立执行边界，也不能由 B0 的结构测试推出 OS containment 已成立。

B1 拥有不可变 `HostContainmentAssessment`、append-only `HostContainmentValidityRecord` 及其 package-local trace producer；B0 验证对应 attestation，CF1 只消费已验证 ref 和 freshness cursor，不建立第二套宿主支持事实。

### 2.4 Capability Factory
...
Resolver、authorization claim 和 B1 dispatch 使用同一 freshness contract：调用方提交预期的 host/bundle/evidence/admission validity revisions，服务端检查“仍为最新且 valid”。claim 后不得用“检查一次再启动”的非原子序列；CF4 必须在共享 control stream 中原子比较全部 validity cursors 并写 `DispatchReservation`。先提交的撤销阻止 reservation，先提交的 reservation 获得唯一启动权；已越过该序列化点的历史 Run 不被改写，紧急终止由独立运行安全策略处理。
...
首个表格型 Adapter vocabulary 可只接受 `column | columns | scalar`。要求 graph、tensor、sparse matrix、event stream，或 `test | transform | solve` 等未注册 operation 时，Requirement/Resolver 返回类型化 capability gap；不得把输入压平、把 operation 改名或伪造 `fit/predict`。后续扩展必须同时版本化 requirement、Adapter input contract、operation vocabulary、harness、evidence protocol 与消费者 schema。B0 不理解这些 vocabulary，只绑定其 contract digest。
...
### 5.4 Harness ABI 由 B1/CF3 版本化

Capability operation dispatch 由可信类型化适配器完成。适配器将选定 operation 及其已校验参数编码进 `capability_spec`，由 B1 broker 使用版本化 harness ABI 调用作者入口：
...
每个 execution attestation 必须绑定 exact harness ABI、input contract 和 output contract digest。升级 ABI 产生新 revision，不能原地改变既有 bundle。B0 不调用作者入口，也不解释 facets；B1 负责密封执行，CF3 负责语义校验。作者入口只能接收服务端已绑定、已密封的输入，且只能返回已注册 Adapter 允许的有界 facets。作者不能：
...
证据等级由 CF3 的服务端验证协议派生，B0 只验证最终 evidence attestation 的 authority、绑定与 freshness：
...
| B1 | `host_containment.assessed` | `subject_ref`, `assessment_ref`, `outcome` |
| B1 | `host_containment.validity.changed` | `assessment_ref`, `validity_ref`, `to_status` |
