...
1. **通用契约与身份**
   ...
   - pre-execution input declaration、server-owned sample-selection policy、post-execution claim 验证与可信引用重写
...
3. **严格运行边界**
   ...
   - host-read canary
   - 不可变 `HostContainmentAssessment`、append-only `HostContainmentValidityRecord` 与真实执行前身份重验
...
- 首版只支持 `column | columns | scalar` 与已注册 operation；非表格输入或未注册 operation 必须返回 typed gap，不能压平或伪装。
- 作者提供的 sample/index claim 必须由服务端对照输入声明和 selection policy 验证，再重写为可信 identity；执行前不预言运行期样本。
- B0 v1 每次 invocation 只允许一个分析样本；多样本 facet 以稳定 typed gap 拒绝。
- 没有目标支持宿主上的真实完整 canary 证据时，不能关闭 supported-host 验收。
...
- statistical protocol 的最小重复数、MCSE/区间、误接受率与 tolerance ceiling 由服务端控制，作者不能放宽。
- E1 无法越权为 verified 或 source-eligible。
- bundle 任一身份成分改变都会改变 digest。
- 作者 sample/index ref 只有在精确 observation identity 验证后才能被服务端重写并用于输出。
- 目标支持宿主上的 sentinel 不可读、不可枚举；网络和越界写入不可用；评估不可变、有效性 append-only 且执行前重验。
- 子进程树、CPU、内存、PID、墙钟与输出总量受策略约束；保证不足时拒绝。
- 非表格 input kind 与未注册 operation 返回稳定 typed gap。
