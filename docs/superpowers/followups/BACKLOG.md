...
| 🟢 主线 | V1.8.5-TYPED-MEMORY | 分级 typed memory（补 `project_domain_fact` 类 + `apply_mode`/`vocabulary_version`/`verifier`/`last_validated_at`） + 模型族契约（抽 `ModelFamilyContract`，workflow-executable 扩到 cs_did/sa_did/dcdh）。**原标 v1.8.4，2026-07-31 改标**——实际 v1.8.4 是 Agent Model Composition，版本号被复用 | Agent 记忆 / 工作流 | 见 `specs/2026-07-31-v1.8.5-typed-memory-and-model-family-design.md`；执行者两层（运行时判定保正确性 + gate preflight 保可见性且不阻塞）；六层检索链**不在本版** | 否 | `specs/2026-07-31-v1.8.5-typed-memory-and-model-family-design.md` |
...
| 🟠 本版遗留 | V1.8.4-TIMER-EVIDENCE | 规划计时器跨组件重挂的修复仍只有确定性测试证据，无真机复现（需在真实规划调用进行中触发重挂） | 验收证据 | 下次真机 Notebook 验收时顺手构造一次重挂；不要补写为已验收 | 否 | `docs/superpowers/handoff/2026-07-30-v1.8.4-remediation-handoff.md` §未完成 |
