...
| 🔵 待立项 | V1.8.3-CAPABILITY-EXT | Agent 提议扩展能力（依赖或自研）；**运行时禁止任意安装**。缺一层判据：无 oracle 时如何验证 | 能力扩展 | 分层判据：有 oracle→逐元素对照；无 oracle 但有可推导性质→性质/模拟回收检验；两者皆无→**只能停在 experimental，永不升 verified** | 否 | roadmap §4 |
