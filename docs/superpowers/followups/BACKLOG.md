...
| 🟠 本版遗留 | V1.8.4-OPEN | 仅两条原 incident 仍开：`notebook_load_not_found` 的 legacy fixture 加载/编译未复现原故障；`chain_agent_proposal_upstream_error` 的 DeepSeek V4 Pro 原始 upstream 故障未取得新的可归因复现。另：当前 macOS 的 `sandbox-exec` 被系统拒绝，令 full gate host-limited，但不是这两条产品 incident 的替代关闭理由。 | Agent/Notebook/宿主 | legacy 不猜根因改代码；provider 故障需在可观测部署/供应商窗口再复现；native containment 仅在支持宿主验收，继续 fail-closed | 否 | `docs/superpowers/handoff/2026-07-30-v1.8.4-remediation-handoff.md` |
