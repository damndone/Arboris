...
| 🟠 本版遗留 | V1.8.4-OPEN | v1.8.4（Agent Model Composition）收尾时仍开着的项：`notebook_load_not_found` 未复现也未修；`panel_workflow_model_composition` 面板固定效应组合；`global_agent_model_figure_evidence` / `predictor_residual_diagnostic_evidence` / `global_agent_workflow_answer_path` 三条 Agent 证据路径；`covariance_comparison_interpretation_boundary`；`notebook_workflow_dependency_shape_correction`；`chain_agent_proposal_upstream_error`；`proposal_ready_budget_terminal_error`；两条浏览器策略 acceptance 阻塞 | Agent/Notebook | 逐条以 FMS 事件为准，不要凭 backlog 行推断状态；开新线时把对应 incident_id 带上 | 否 | `.agent/devlines/v1-8-4-agent-model-composition/events.jsonl` |
| 🟠 本版遗留 | V1.8.4-TIMER-EVIDENCE | 规划计时器跨组件重挂的修复只有确定性测试证据，无真机复现（需在真实规划调用进行中触发重挂） | 验收证据 | 下次真机 Notebook 验收时顺手构造一次重挂；不要补写为已验收 | 否 | `docs/superpowers/handoff/2026-07-31-v1.8.4-release-handoff.md` §未完成 |
