...
| 🔵 AI | V1 | A3:逐 artifact「Explain」按钮（drawer artifact 行→带该 artifact 上下文直达 Ask AI） | 前端+packet | 复用 askAiAboutNode 通道,packet 加 artifact 聚焦字段 | 否 | §2-V1 |
| 🔵 AI | V2 | A4:LLM 设置面板（`GET /llm/config` 脱敏回显 base_url/model,前端只读展示;换 key 仍走 env） | 前后端 | 薄端点+设置区块;不做前端存 key | 否 | §2-V2 |
| 🔵 AI | V3 | C2:底部面板做实——AI 交互留痕 tab（AskAI/报告生成记录,通往 v1.7 带类型可检查操作记录） | 前端 | 先只读时间线,复用 reportHistory 思路;三个 stale "lands in V1.5.3" tab 一并处置 | 否 | §2-V3 |
| 🟡 兼容 | V4 | 旧格式项目（v1.5.x,如 `~/Desktop/安全边际筛选`）`/graph` scanner 挂住不返回 | 后端兼容 | 定位 scanner 挂点,legacy 项目应 degrade 而非 hang（非回归,待查） | 是(旧项目) | §2-V4 |
| 🔵 AI | V5 | Ask AI 输出裸 markdown（`###`/`**` 原样显示,图表零碎不美观）（反馈批3,2026-07-12） | 前端 | 面板内渲染 md（Report 视图渲染管线可复用） | 否 | — |
| 🔵 AI | V6 | Ask AI 无历史记录（改问重生成即覆盖旧答案）（反馈批3） | 前端 | 照 reportHistory 模式做 per-node 问答历史;与 V3 留痕 tab 同素材 | 否 | — |
...
| 🟡 前端 | V8 | 画布缺「复位」键:节点拖乱后无法一键回自动布局（反馈批3） | 前端 | GraphCanvas 加 re-layout 复位按钮(layout seed 重置+fit) | 否 | — |
| 🟡 待查 | V9 | 变量节点外层折叠框「又没了」:变量多时纵向爆屏(fold 阈值=严格>3;疑 forest 投影 remap 后 folding 失效,回归待查)（反馈批3） | 前端 bug | 真机复现多变量项目,查 foldVariableClusters 是否吃到 remap 后 parent key | 是(多变量) | — |
| 🔵 拍板 | V10 | Home 界面只剩新建项目,信息密度太低;整合进 workbench（反馈批3） | 产品拍板 | topbar 项目切换器带「最近项目+新建」,Home 退役或重定向 | 否 | — |
