...
|---|---|---|---|---|---|---|
| 🟡 工程 | N1 | `http/drafts_routes.py` 819 行 > spec「<500」目标（含 `_execute_genesis_draft` 166 + `execute_pipeline_draft` 200 的重编排 handler） | 架构（v1.6.10 遗留） | 把执行编排下沉 `services/draft_service.py`，留薄 handler（drafts_routes 可降到 ~450） | 否 | §2-N1 |
...
| 🟢 主线 | M1 | **v1.6.11+** 图内任选对比 / 节点 Ask AI / 引用报告（北极星实质步；v1.6.10 改做 D1 后顺延） | 主线 | **先定 Ask AI 的 `/llm/chat` 模型选型 = roadmap 开放问题 #3（未决）** | — | roadmap |
...
### §2-N1 · `http/drafts_routes.py` 819 行 > spec「<500」目标（v1.6.10 D1 遗留）

D1 拆分把 drafts 簇搬进 `http/drafts_routes.py`,但该簇含两个重编排 handler
（`_execute_genesis_draft` 166 行 + `execute_pipeline_draft` 200 行）——这些"胖 handler"正是
D1 要治的承重容器病（只是缩小版）。按分层意图,执行编排该下沉 `services/draft_service.py`,
route 只留薄壳（解析 → 调 service → 包装响应）。做完 drafts_routes 可降到 ~450 < 500。
其余 http/ 模块均已 <400。非阻塞,cohesive 且功能正确。
