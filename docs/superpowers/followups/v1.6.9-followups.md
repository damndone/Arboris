...
**v1.6.12 进度（2026-07-12）**：✅ V1/V2/V3/V5/V6/V8/V10 已在 `workbench-v1.6.12` 实现；V4 已在本地 closeout 修复并通过独立 legacy `/graph` smoke。V9 已销案(非 bug)。V2 现为可写的多供应商设置管理器（含 1M 上下文元数据、模型映射、探活与脱敏预览）；V10 现为 Workbench 内 Home 视图（含最近项目、创建项目、LLM 状态与设置入口），`/?home=1` 仅作兼容别名。最新完整 gate 需在本次收尾后重跑；发版流程未走:review→PR→merge main→tag v1.6.12。
