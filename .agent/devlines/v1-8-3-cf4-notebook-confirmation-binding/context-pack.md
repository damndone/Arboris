...
Line: `v1-8-3-cf4-notebook-confirmation-binding`
Baseline SHA: `b44060243b95800f32456588a5d4f1f78c6601fb`
...
## Boundary
- Affected paths: `backend/workbench/contracts/agent/notebook_option.py`, `backend/workbench/agent/notebook/service.py`, `tests/test_capability_notebook_binding.py`, `tests/test_notebook_capability_binding.py`, `tests/test_notebook_routes.py`
- Allowed paths: `backend/workbench/contracts/agent/notebook_option.py`, `backend/workbench/agent/notebook/service.py`, `tests/test_capability_notebook_binding.py`, `tests/test_notebook_capability_binding.py`, `tests/test_notebook_routes.py`
- Protected paths: `backend/workbench/app.py`, `backend/workbench/agent/trace.py`, `backend/workbench/agent/context_compiler.py`, `frontend/src/notebook/NotebookSurface.tsx`, `frontend/src/notebook/notebook.css`, `backend/workbench/agent/notebook/materialization.py`, `backend/workbench/capability_factory/execution_authorization.py`
- Dependencies: `docs/superpowers/specs/2026-07-25-v1.8.3-custom-capability-runtime-design.md`, `docs/superpowers/specs/2026-07-25-v1.8.3-capability-factory-design.md`, `backend/workbench/capability_factory/notebook_binding.py`, `backend/workbench/capability_factory/binding_catalog.py`, `backend/workbench/capability_factory/execution_authorization.py`
- Tests: `LC_ALL=en_US.UTF-8 LANG=en_US.UTF-8 PYTHONPATH=backend .venv/bin/pytest -q tests/test_capability_notebook_binding.py tests/test_notebook_capability_binding.py tests/test_notebook_routes.py tests/test_capability_execution_authorization.py tests/test_no_exercise_specific_naming.py`
- Known gates: `new worktree dependencies must be linked with bash scripts/link-shared-deps.sh <absolute-worktree-path>`, `native containment canary is host-capability evidence and must not be treated as passed on this macOS host`, `full gate must be run without piping tsc output to tail`, `no new automatic execution surface; this slice only validates and persists user-confirmed authorization`, `no dynamic dependency installation or untrusted process/network/host filesystem access`
