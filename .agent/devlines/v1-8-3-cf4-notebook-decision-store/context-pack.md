...
Line: `v1-8-3-cf4-notebook-decision-store`
Baseline SHA: `4d7a84a3322aa98a4810ac5e881c4fa733be6015`
...
## Boundary
- Affected paths: `backend/workbench/agent/notebook/recommendation.py`, `backend/workbench/agent/notebook/store.py`, `backend/workbench/agent/storage.py`, `tests/test_notebook_recommendation.py`, `tests/test_notebook_store_append_only.py`
- Allowed paths: `backend/workbench/agent/notebook/recommendation.py`, `backend/workbench/agent/notebook/store.py`, `backend/workbench/agent/storage.py`, `tests/test_notebook_recommendation.py`, `tests/test_notebook_store_append_only.py`
- Protected paths: `backend/workbench/agent/notebook/service.py`, `backend/workbench/agent/supervisor.py`, `backend/workbench/capability_factory/dispatch.py`, `backend/workbench/app.py`, `frontend/src`, `scripts`
- Dependencies: `docs/superpowers/specs/2026-07-25-v1.8.3-capability-factory-design.md`, `docs/superpowers/specs/2026-07-25-v1.8.3-custom-capability-runtime-design.md`, `.agent/devlines/v1-8-3-cf4-notebook-feasibility/RETROSPECTIVE.md`, `backend/workbench/agent/notebook/store.py`, `backend/workbench/agent/notebook/recommendation.py`
- Tests: `LC_ALL=en_US.UTF-8 LANG=en_US.UTF-8 PYTHONPATH=backend .venv/bin/pytest -q tests/test_notebook_recommendation.py tests/test_notebook_store_append_only.py`, `LC_ALL=en_US.UTF-8 LANG=en_US.UTF-8 PYTHONPATH=backend .venv/bin/pytest -q tests/test_no_exercise_specific_naming.py`
- Known gates: `run bash scripts/link-shared-deps.sh with an absolute worktree path before any valid gate claim`, `naming gate rejects exercise-specific names and fixed years in product code`, `do not pipe tsc to tail before reading its exit code`, `no full gate or production integration claim unless explicitly run`
