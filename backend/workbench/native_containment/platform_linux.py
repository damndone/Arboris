"""Linux namespace/bwrap canary adapter with independent host status."""
...
"""
    ...
    def __init__(
        ...
        *,
        ...
        self.host_supported = host_supported or (lambda: platform.system() == "Linux")
    ...
    def discover_identity(self) -> HostIdentity:
        ...
        if not path.is_absolute() or not path.is_file():
            raise OSError("Linux containment backend is unavailable")
        return HostIdentity(
    ...
    def run(self, policy: ContainmentPolicy) -> CanaryResult:
        ...
        return CanaryResult("unsupported", self._probe_reason or "NATIVE_CONTAINMENT_CANARY_FAILED", assertions)
    ...
    def _default_probe(self, policy: ContainmentPolicy) -> Mapping[str, bool]:
        with tempfile.TemporaryDirectory(prefix="workbench-native-canary-") as root_name:
            ...
            for case in self.CANARY_CASES:
                results[case] = self._run_case(policy, case, output_root)
            return results
    ...
    def _preexec(self, policy: ContainmentPolicy):
        ...
        def apply_limits() -> None:  # pragma: no cover - runs in the child process
            os.setsid()
            cpu_seconds = max(1, math.ceil(budget.cpu_millis / 1000))
    ...
    def _run_case(self, policy: ContainmentPolicy, case: str, output_root: Path) -> bool:
        ...
        for key, value in self._environment(policy).items():
            argv.extend(("--setenv", key, value))
        argv.extend((self.python_executable, "-I", "-c", self._CANARY_SCRIPT, case, str(output_root)))
