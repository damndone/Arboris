...
def _model_from_dict(raw: Any) -> ModelRecord:
    ...
    if context_window_tokens is not None and (
        isinstance(context_window_tokens, bool) or not isinstance(context_window_tokens, int)
    ):
    ...
    return ModelRecord(
        ...
        request_model=_required_string(raw, "request_model"),
...
def _timeout_value(value: Any) -> float:
    ...
    try:
        ...
        raise ValueError("timeout_s must be numeric") from exc
