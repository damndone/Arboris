...
def test_nested_panel_ols_covariance_is_bound_and_used(tmp_path):
    run_root, result = _run(
        tmp_path, mode="auto", y="profit", x=["rnd"],
        model_type="panel_ols", entity_col="firm", time_col="yr",
        covariance="robust", model_options={"covariance": "clustered"},
    )
    assert result["status"] == "completed"
    inputs = read_json(run_root / "run_inputs.json")
    model = read_json(run_root / "model_results" / "panel_ols_1.json")
    assert inputs["form"]["covariance"] == "clustered"
    assert inputs["form"]["model_options"] == {"covariance": "clustered"}
    assert inputs["form"]["model_options_binding"]["owner_model_type"] == "panel_ols"
    assert model["covariance"] == "clustered"
